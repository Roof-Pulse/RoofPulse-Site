...
VERSION      = "0.1.0"
LAUNCH_DATE  = date(2026, 4, 1)   # update to your actual go-live date
...
leads = []  # live demo simulations accumulate here


def days_live():
    return max(1, (date.today() - LAUNCH_DATE).days)


def recovered_count():
    return len(DEMO_LEADS) + len(leads)
...
<div class="w">
  <div class="hd"><h1>RoofPulse v{VERSION}</h1><h2>🔥 New Pilot Request</h2></div>
  <div class="bd">
  ...
  <div class="ft">RoofPulse v{VERSION} · Collierville, TN · Built by Hayden Keithley</div>
</div></body></html>"""
...
<div class="w">
  ...
  <div class="bd">
    <p class="greet">Hey {first} — this is Hayden, not an auto-reply bot. I'm a local developer in Collierville, TN and I personally set up every system. I'll be reaching out to you at <strong>{company}</strong> within the next few hours.</p>
...
def simulate_call():
    ...
    leads.append({
        ...
        "status": "New", "source": "Missed Call",
        "value": 0, "time": "just now"
    })
    return jsonify({"success": True, "recovered": recovered_count(), "days_live": days_live()})

@app.route('/api/stats', methods=['GET'])
def stats():
    return jsonify({"recovered": recovered_count(), "days_live": days_live(), "version": VERSION})
...
def contact():
    ...
    send_async(OWNER_EMAIL, f"🔥 New Pilot Request: {name} — {company}",
               owner_email(name, company, phone, email, missed_calls, submitted_at))
    if email:
        send_async(email, f"You're confirmed, {name.split()[0]}! — RoofPulse",
                   prospect_email(name, company))
